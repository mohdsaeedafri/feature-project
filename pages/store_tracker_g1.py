    ...
    with st.form("filters_form"):
        ...
        with col1:
            with st.expander("Date Range", expanded=False):
                # Clean years list to remove 0 and sort
                valid_years = [year for year in initial_options['years'] if year != 0]
                valid_years = sorted(valid_years)

                # --- First row: Start Year + Start Week ---
                start_col1, start_col2 = st.columns(2)

                with start_col1:
                    start_year_idx = valid_years.index(current_year) if current_year in valid_years else 0
                    st.session_state.filters["start_year"] = st.selectbox(
                        "Start Year", 
                        valid_years, 
                        index=start_year_idx,
                ...
                # --- Second row: End Year + End Week ---
                end_col1, end_col2 = st.columns(2)

                with end_col1:
                    end_year_options = [year for year in valid_years if year >= st.session_state.filters["start_year"]]
                    end_year_idx = end_year_options.index(current_year) if current_year in end_year_options else 0
                ...
                with end_col2:
                    if st.session_state.filters["end_year"] == st.session_state.filters["start_year"]:
                        end_week_options = list(range(st.session_state.filters["start_week"], 54))
                        end_week_idx = min(current_week - st.session_state.filters["start_week"], len(end_week_options) - 1)
                    else:
                        end_week_options = list(range(1, 54))
                        end_week_idx = current_week - 1
        ...
        # Get dynamic options based on date range
        date_filtered = pd.concat([closings_df, openings_df])
        date_filtered['year'] = date_filtered['year'].astype(int)
        date_mask = (
            (date_filtered['year'].astype(int) > st.session_state.filters["start_year"]) |
            ((date_filtered['year'].astype(int) == st.session_state.filters["start_year"]) & 
            (date_filtered['Week_Number'] >= st.session_state.filters["start_week"]))
        ) & (
            (date_filtered['year'].astype(int) < st.session_state.filters["end_year"]) |
            ((date_filtered['year'].astype(int) == st.session_state.filters["end_year"]) & 
            (date_filtered['Week_Number'] <= st.session_state.filters["end_week"]))
        )
        date_filtered = date_filtered.loc[date_mask]
        
        # Clean filter options to remove 0 and None values
        ...
        dynamic_options['sectors'] = ["All"] + clean_options(dynamic_options['sectors'][1:])
        dynamic_options['parent_companies'] = ["All"] + clean_options(dynamic_options['parent_companies'][1:])
        dynamic_options['companies'] = ["All"] + clean_options(dynamic_options['companies'][1:])
        dynamic_options['countries'] = ["All"] + clean_options(dynamic_options['countries'][1:])
        
        with col2:
            with st.expander("Sector", expanded=False):
                st.session_state.filters["sector"] = st.selectbox(
                    "Select Sector", 
                    dynamic_options['sectors'], 
                    index=0,
                    key="sector_select"
                )
        
        # Get options based on date + sector
        ...
        with col3:
            with st.expander("Retailers", expanded=False):
                st.session_state.filters["parent_companies"] = st.multiselect(
                    "Parent Companies", 
                    dynamic_options['parent_companies'],
                    default=["All"],
                    key="parent_companies_select"
                )
                
                st.session_state.filters["companies"] = st.multiselect(
                    "Brands/Chains", 
                    dynamic_options['companies'],
                    default=["All"],
        ...
        if submitted:
            # Validate date range
            if (st.session_state.filters["end_year"] < st.session_state.filters["start_year"]) or \
            (st.session_state.filters["end_year"] == st.session_state.filters["start_year"] and \
                st.session_state.filters["end_week"] < st.session_state.filters["start_week"]):
